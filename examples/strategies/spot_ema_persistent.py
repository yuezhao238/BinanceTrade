...
from binance_trade.builtin_strategies import Candle, closes, ema_series
...
class SpotEmaPersistentStrategy:
    symbol: str = "BTCUSDT"
    interval: str = "1h"
    fast_period: int = 20
    slow_period: int = 50
    ...
    def __post_init__(self) -> None:
        ...
        self._candles = deque(maxlen=max(self.warmup_bars, self.slow_period + 20))

    def market_streams(self) -> list[str]:
        return [f"{self.symbol.lower()}@kline_{self.interval}"]

    async def on_start(self, ctx: StrategyContext) -> None:
        history = await ctx.get_klines(self.symbol, self.interval, limit=max(self.warmup_bars, self.slow_period + 20))
        for item in history:
        ...
        ctx.logger.info(
            "persistent ema strategy started symbol=%s interval=%s position_qty=%s warmup=%s",
            self.symbol,
            self.interval,
            self._position_qty,
            len(self._candles),
    ...
    async def on_market_event(self, ctx: StrategyContext, event: StrategyEvent):
        ...
        if len(self._candles) < self.slow_period + 2:
            return None
        ...
        if signal > 0 and self._position_qty <= 0:
            ctx.logger.info("ema bullish crossover -> market buy symbol=%s quote=%s", self.symbol, self.quote_order_qty)
            return ctx.market_buy(self.symbol, quote_order_qty=self.quote_order_qty)

        if signal < 0 and self._position_qty > 0:
            ctx.logger.info("ema bearish crossover -> market sell symbol=%s quantity=%s", self.symbol, self._position_qty)
    ...
    def _compute_signal(self) -> int | None:
        series = closes(list(self._candles))
        fast = ema_series(series, self.fast_period)
        slow = ema_series(series, self.slow_period)
        if len(fast) < 2 or len(slow) < 2:
            return None
        prev_fast, fast_now = fast[-2], fast[-1]
        prev_slow, slow_now = slow[-2], slow[-1]
...
def create_strategy(**kwargs):
    return SpotEmaPersistentStrategy(**kwargs)
